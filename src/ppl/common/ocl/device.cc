...
bool Device::detectDevices() {
    ...
    cl_int error_code;
    cl_uint num_devices;
    error_code = clGetDeviceIDs(platform_ids_[platform_index_],
                                CL_DEVICE_TYPE_GPU, 0, nullptr, &num_devices);
    if (error_code != CL_SUCCESS ) {
...
bool Device::queryPlatformInfo(const cl_platform_id& platform_id,
    ...
    error_code = clGetPlatformInfo(platform_id, param_name, 0, nullptr,
                                   &returned_size);
    if (error_code != CL_SUCCESS) {
...
bool Device::getPlatformInfos() {
    ...
    bool succeeded;
    succeeded = queryPlatformInfo(platform_ids_[platform_index_],
                                  CL_PLATFORM_NAME, platform_name_);
    if (!succeeded) {
        return false;
    }

    succeeded = queryPlatformInfo(platform_ids_[platform_index_],
                                  CL_PLATFORM_VENDOR, platform_vendor_);
    if (!succeeded) {
        return false;
    }

    succeeded = queryPlatformInfo(platform_ids_[platform_index_],
                                  CL_PLATFORM_PROFILE, platform_profile_);
    if (!succeeded) {
        return false;
    }

    succeeded = queryPlatformInfo(platform_ids_[platform_index_],
                                  CL_PLATFORM_VERSION, platform_version_);
    if (!succeeded) {
        return false;
    }

    succeeded = queryPlatformInfo(platform_ids_[platform_index_],
                                  CL_PLATFORM_EXTENSIONS, platform_extensions_);
    if (!succeeded) {
        return false;
    }
...
bool Device::queryDeviceInfo(const cl_device_id& device_id,
    ...
    cl_int error_code;
    size_t returned_size;
    error_code = clGetDeviceInfo(device_id, param_name, 0, nullptr,
                                 &returned_size);
    if (error_code != CL_SUCCESS) {
...
int Device::checkOpenCLVersion(const cl_device_id& device_id) {
    ...
    bool succeeded = queryDeviceInfo(device_id, CL_DEVICE_OPENCL_C_VERSION,
...
#define QUERY_DEVICE_INFO(succeeded, device_id, param_name, param_value,       \
                          scaling)                                             \
...
bool Device::getDeviceBasicInfos(const cl_device_id& device_id) {
    ...
    bool succeeded;
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_VENDOR, device_vendor_,
                      true);
    if (device_vendor_.find("QUALCOMM") != std::string::npos) {
        gpu_type_ = ADRENO_GPU;
    }
    ...
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_GLOBAL_MEM_SIZE, elements,
                      false);
    global_mem_size_ = *((cl_ulong*)elements.data());

    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_IMAGE_SUPPORT, elements,
                      false);
    ...
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_MAX_SAMPLERS, elements,
                      false);
    max_samplers_ = *((cl_uint*)elements.data());

    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_LOCAL_MEM_TYPE, elements,
                      false);
    local_mem_type_ = *((cl_device_local_mem_type*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_LOCAL_MEM_SIZE, elements,
                      false);
...
bool Device::getDeviceThoroughInfos(const cl_device_id& device_id) {
    ...
    bool succeeded;
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_NAME, device_name_,
                      true);
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_PROFILE, device_profile_,
                      true);
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_VERSION, device_version_,
                      true);
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DRIVER_VERSION, driver_version_,
                      true);
    ...
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE, elements, false);
    native_vector_width_double_ = *((cl_uint*)elements.data());

    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_CHAR, elements, false);
    preferred_vector_width_char_ = *((cl_uint*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_SHORT, elements, false);
    preferred_vector_width_short_ = *((cl_uint*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT, elements, false);
    preferred_vector_width_int_ = *((cl_uint*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_LONG, elements, false);
    preferred_vector_width_long_ = *((cl_uint*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF, elements, false);
    preferred_vector_width_half_ = *((cl_uint*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT, elements, false);
    preferred_vector_width_float_ = *((cl_uint*)elements.data());
    QUERY_DEVICE_INFO(succeeded, device_id,
                      CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE, elements, false);
    ...
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_ADDRESS_BITS, elements,
                      false);
    ...
    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_ENDIAN_LITTLE, elements,
                      false);
    endian_little_ = *((cl_bool*)elements.data());

    QUERY_DEVICE_INFO(succeeded, device_id, CL_DEVICE_AVAILABLE, elements,
                      false);
